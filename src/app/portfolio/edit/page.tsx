...
export default function EditPortfolio() {
  ...
  async function handleFileUpload(event: React.ChangeEvent<HTMLInputElement>) {
    try {
      ...
      alert('Error uploading image! Ensure the "portfolio-assets" bucket exists in Supabase.')
      console.error(error)
    } finally {
      setUploading(false)
    }
  ...
  async function updateSkill(id: number, name: string) {
    const updated = skills.map(s => s.id === id ? { ...s, skill_name: name } : s)
    setSkills(updated)
  ...
  async function addExperience() {
    const newExp = { 
      company: 'New Institution', role: 'Researcher', 
  ...
  async function addProject() {
    const newProj = { title: 'New AI Project', link: '#', description: 'Built an end-to-end pipeline...' }
  ...
  async function deleteProject(id: number) {
    if (!confirm('Permanently remove this project from your portfolio?')) return
  ...
  return (
    <div className="max-w-[800px] mx-auto pt-12 pb-24 px-6">
      <div className="flex justify-between items-center mb-16 border-b border-gray-100 pb-8">
        ...
        <button onClick={saveProfile} className="bg-[#1a8917] text-white px-8 py-2 rounded-full text-sm font-medium hover:bg-[#156d12] transition-all shadow-sm">
          Save Profile
        </button>
      </div>
      ...
      <section className="mb-20 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row gap-8 items-center bg-gray-50 p-8 rounded-3xl mb-8 border border-gray-100">
          <div className="relative group w-32 h-32 flex-shrink-0">
            <img 
              ...
              className="w-32 h-32 rounded-full object-cover border-4 border-white shadow-md group-hover:opacity-40 transition-all bg-gray-200"
              alt="Preview"
            />
            <button 
              onClick={() => fileInputRef.current?.click()}
              className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 text-[10px] font-black uppercase tracking-tighter text-black text-center px-2"
            >
          ...
          <div className="flex-grow w-full space-y-4">
            ...
            <input placeholder="Direct Image URL (Optional)" className="w-full p-4 bg-white border border-gray-100 rounded-xl outline-none text-[10px] text-gray-400 font-mono" value={profile.image_url || ''} onChange={e => setProfile({...profile, image_url: e.target.value})} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <input placeholder="CV Link" className="p-4 bg-gray-50 rounded-xl outline-none text-sm" value={profile.cv_link || ''} onChange={e => setProfile({...profile, cv_link: e.target.value})} />
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          {skills.map(skill => (
            <div key={skill.id} className="group relative bg-white border border-gray-100 px-4 py-2 rounded-full flex items-center gap-2 hover:border-[#1a8917]/30 transition-all">
              ...
              <button onClick={() => deleteSkill(skill.id)} className="text-gray-300 hover:text-red-500 text-xs font-bold">×</button>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-20">
        ...
        <div className="space-y-8">
          {experiences.map(exp => (
            <div key={exp.id} className="p-8 bg-gray-50 rounded-3xl border border-gray-100 group relative transition-all">
              ...
              <textarea 
                className="w-full bg-white border border-gray-100 p-4 rounded-xl text-sm font-serif text-gray-600 leading-relaxed outline-none"
                rows={4}
        ...
        <div className="space-y-8">
          {projects.map(proj => (
            <div key={proj.id} className="p-8 border border-gray-100 rounded-3xl group relative transition-all">
              ...
              <textarea 
                className="w-full bg-transparent border-b border-gray-100 py-2 text-[15px] text-gray-500 font-serif leading-relaxed outline-none"
                rows={3}
