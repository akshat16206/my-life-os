...
export default function EditPortfolio() {
  ...
  async function handleFileUpload(event: React.ChangeEvent<HTMLInputElement>) {
    try {
      ...
      alert('Error uploading image! Make sure you created the "portfolio-assets" bucket in Supabase.')
      console.error(error)
    } finally {
      setUploading(false)
    }
  ...
  async function addSkill() {
    ...
    if (!error) setSkills([...skills, data[0]])
  }
  ...
  async function updateExperience(id, field, value) {
  ...
  async function addProject() {
    ...
    if (!error) setProjects([data[0], ...projects])
  }
        ...
        <div className="flex flex-wrap gap-3">
          {skills.map(skill => (
            <div key={skill.id} className="group relative bg-white border border-gray-100 px-4 py-2 rounded-full flex items-center gap-2 hover:border-[#1a8917]/30 transition-all">
              <input 
                className="bg-transparent text-sm font-medium outline-none text-gray-700 w-24"
