...
export default function EditPortfolio() {
  ...
  if (loading) return <div className="p-20 text-center font-serif italic text-gray-400">Syncing Research Data...</div>
  ...
  return (
    <div className="max-w-[800px] mx-auto pt-12 pb-24 px-6">
      ...
      <section className="mb-20 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row gap-8 items-center bg-gray-50 p-8 rounded-3xl mb-8 border border-gray-100">
          <div className="relative group w-32 h-32 flex-shrink-0">
            <img src={profile.image_url || "/my_image.jpg"} className="w-32 h-32 rounded-full object-cover border-4 border-white shadow-md transition-all" alt="Preview" />
          ...
          <div className="flex-grow w-full space-y-4">
            <input placeholder="Full Name" className="w-full p-4 bg-white border border-gray-100 rounded-xl outline-none text-sm" value={profile.full_name || ''} onChange={e => setProfile({...profile, full_name: e.target.value})} />
      ...
      {/* Skills, Experience, and Projects follow using the same value={... || ''} pattern */}
      <section className="mb-20">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-[11px] font-bold text-gray-400 uppercase tracking-[0.2em]">Technical Stack</h2>
          <button onClick={addSkill} className="text-[#1a8917] text-xs font-bold">+ ADD SKILL</button>
        </div>
        <div className="flex flex-wrap gap-3">
          {skills.map(skill => (
            <div key={skill.id} className="group relative bg-white border border-gray-100 px-4 py-2 rounded-full flex items-center gap-2 transition-all">
              ...
              <button onClick={() => deleteSkill(skill.id)} className="text-gray-300 hover:text-red-500 text-xs">×</button>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-20">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={addExperience} className="text-[#1a8917] text-xs font-bold">+ ADD ROLE</button>
        </div>
        <div className="space-y-8">
          {experiences.map(exp => (
            <div key={exp.id} className="p-8 bg-gray-50 rounded-3xl border border-gray-100 group relative">
              <button onClick={() => deleteExperience(exp.id)} className="absolute -top-3 -right-3 bg-white border border-gray-100 text-red-500 p-2 rounded-full shadow-sm opacity-0 group-hover:opacity-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                <input className="bg-transparent font-bold text-xl serif-title outline-none" value={exp.role || ''} onChange={e => updateExperience(exp.id, 'role', e.target.value)} />
                <input className="bg-transparent text-right text-gray-400 italic outline-none" value={exp.duration || ''} onChange={e => updateExperience(exp.id, 'duration', e.target.value)} />
              </div>
              <input className="w-full bg-transparent text-[#1a8917] font-medium mb-4 outline-none uppercase text-xs" value={exp.company || ''} onChange={e => updateExperience(exp.id, 'company', e.target.value)} />
              <textarea className="w-full bg-white border border-gray-100 p-4 rounded-xl text-sm" rows={4} value={exp.description?.join('\n') || ''} onChange={e => updateExperience(exp.id, 'description', e.target.value.split('\n'))} />
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-20">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={addProject} className="text-[#1a8917] text-xs font-bold">+ ADD PROJECT</button>
        </div>
        <div className="space-y-8">
          {projects.map(proj => (
            <div key={proj.id} className="p-8 border border-gray-100 rounded-3xl group relative transition-all">
              <button onClick={() => deleteProject(proj.id)} className="absolute -top-3 -right-3 bg-white border border-gray-100 text-red-500 p-2 rounded-full shadow-sm opacity-0 group-hover:opacity-100">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18m-2 0v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6m3 0V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>
              </button>
              <input className="w-full text-xl font-bold serif-title mb-2 outline-none" value={proj.title || ''} onChange={e => updateProject(proj.id, 'title', e.target.value)} />
              <input className="w-full text-xs text-[#1a8917] mb-4 outline-none font-mono" value={proj.link || ''} onChange={e => updateProject(proj.id, 'link', e.target.value)} />
              <textarea className="w-full bg-transparent border-b border-gray-100 py-2 text-[15px] text-gray-500 outline-none" rows={3} value={proj.description || ''} onChange={e => updateProject(proj.id, 'description', e.target.value)} />
            </div>
          ))}
        </div>
      </section>
